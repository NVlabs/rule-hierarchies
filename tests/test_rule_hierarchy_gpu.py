...
class RankPreservingTestGPU(unittest.TestCase):
    def setUp(self) -> None:
        rules = [AlwaysWithin(0.0, 2.0), AlwaysWithin(1.0, 3.0)]
        scaling = [1.0, 1.0]
        self.rule_hierarchy = RuleHierarchy[torch.Tensor](rules, scaling)
...
class ContOptimizerTestGPU(unittest.TestCase):
    ...
    def cont_planner(self, rule_hierarchy: RuleHierarchy) -> None:
        ...
        while u_grad_norm > self.grad_tol and itr < self.max_num_itr:
            ...
            loss = rule_hierarchy.diff_traj_cost(traj.to("cuda:0"))
            loss.backward()
            optimizer.step()
    ...
    def test_cont_optimization_feasible(self):
        ...
        if self.vis:
            self.visualize(
                ...
                lower_bound_y,
                upper_bound_y,
                "test_feasible_gpu.png",
            )
        _, scaled_robustness_vector = rule_hierarchy.traj_cost(
            self.traj.to("cuda:0"), get_robustness_vector=True
        )
    ...
    def test_cont_optimization_infeasible_avoid_circle(self):
        ...
        if self.vis:
            self.visualize(
                ...
                lower_bound_y,
                upper_bound_y,
                "test_infeasible_avoid_pothole_gpu.png",
            )
        _, scaled_robustness_vector = rule_hierarchy.traj_cost(
            self.traj.to("cuda:0"), get_robustness_vector=True
        )
    ...
    def test_cont_optimization_infeasible_avoid_boundary(self):
        ...
        if self.vis:
            self.visualize(
                ...
                lower_bound_y,
                upper_bound_y,
                "test_infeasible_stay_within_lane_gpu.png",
            )
        _, scaled_robustness_vector = rule_hierarchy.traj_cost(
            self.traj.to("cuda:0"), get_robustness_vector=True
        )
